...
const FRONTEND_ORIGIN=process.env.FRONTEND_ORIGIN||'https://ramkaranbhambhu201-ship-it.github.io';
const ALLOWED_ORIGINS=new Set([FRONTEND_ORIGIN,'http://localhost:3000','http://127.0.0.1:3000']);
function securityHeaders(){return {'X-Content-Type-Options':'nosniff','X-Frame-Options':'DENY','Referrer-Policy':'strict-origin-when-cross-origin','Permissions-Policy':'camera=(self), microphone=(self), geolocation=(self)','Content-Security-Policy':"default-src 'self' https: data: blob:; script-src 'self' 'unsafe-inline' https:; style-src 'self' 'unsafe-inline' https:; img-src 'self' data: blob: https:; connect-src 'self' https: http://localhost:3000 http://127.0.0.1:3000; frame-ancestors 'none'; base-uri 'self'; form-action 'self'"}}
function corsHeaders(req){const origin=req.headers.origin; const h=securityHeaders(); if(origin&&ALLOWED_ORIGINS.has(origin)){h['Access-Control-Allow-Origin']=origin;h['Access-Control-Allow-Credentials']='true';h['Vary']='Origin'} return h}
function send(res,code,data,type='application/json',req=null,extra={}){const h={'Content-Type':type,...securityHeaders(),...(req?corsHeaders(req):{}),...extra};res.writeHead(code,h);res.end(type.includes('json')?JSON.stringify(data):data)}
function body(req,maxBytes=20*1024*1024){return new Promise((ok,fail)=>{let s='',tooBig=false;req.on('data',c=>{if(tooBig)return;s+=c;if(Buffer.byteLength(s)>maxBytes){tooBig=true;fail(new Error('Request body too large'));req.destroy()}});req.on('end',()=>{if(tooBig)return;try{ok(s?JSON.parse(s):{})}catch(e){fail(new Error('Invalid JSON'))}});req.on('error',fail)})}

const USERS_DB=path.join(__dirname,'users.json');
const sessions=new Map();
const rateBuckets=new Map();
function readUsers(){try{return JSON.parse(fs.readFileSync(USERS_DB,'utf8'))}catch(e){return {users:[]}}}
function writeUsers(d){fs.writeFileSync(USERS_DB,JSON.stringify(d,null,2),{mode:0o600})}
function normalizeEmail(x){return String(x||'').trim().toLowerCase()}
function validEmail(x){return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(x)&&x.length<=254}
function hashPassword(password,salt=crypto.randomBytes(16).toString('hex')){const hash=crypto.scryptSync(password,salt,64,{N:16384,r:8,p:1}).toString('hex');return {salt,hash}}
function verifyPassword(password,stored){try{const got=Buffer.from(hashPassword(password,stored.salt).hash,'hex'),want=Buffer.from(stored.hash,'hex');return got.length===want.length&&crypto.timingSafeEqual(got,want)}catch(e){return false}}
...
function currentUser(req){const token=parseCookies(req).vn_session;const s=token&&sessions.get(token);if(!s)return null;if(s.expires<Date.now()){sessions.delete(token);return null}return s.user}
function setSession(res,user){const token=crypto.randomBytes(32).toString('base64url');sessions.set(token,{user,expires:Date.now()+7*24*60*60*1000});res.setHeader('Set-Cookie',`vn_session=${encodeURIComponent(token)}; Max-Age=604800; Path=/; HttpOnly; Secure; SameSite=None`);return token}
function clearSession(req,res){const token=parseCookies(req).vn_session;if(token)sessions.delete(token);res.setHeader('Set-Cookie','vn_session=; Max-Age=0; Path=/; HttpOnly; Secure; SameSite=None')}
function requireAuth(req,res){const u=currentUser(req);if(!u){send(res,401,{ok:false,error:'Authentication required',code:'AUTH_REQUIRED'},'application/json',req);return null}return u}
function rateLimit(req,key,limit=60,windowMs=60000){const ip=String(req.headers['x-forwarded-for']||req.socket.remoteAddress||'unknown').split(',')[0].trim();const k=ip+'|'+key;const now=Date.now();let b=rateBuckets.get(k);if(!b||b.reset<now)b={count:0,reset:now+windowMs};b.count++;rateBuckets.set(k,b);return {ok:b.count<=limit,retry:Math.max(1,Math.ceil((b.reset-now)/1000))}}
function rateGuard(req,res,key,limit,windowMs=60000){const r=rateLimit(req,key,limit,windowMs);if(!r.ok){res.setHeader('Retry-After',String(r.retry));send(res,429,{ok:false,error:'Too many requests. Please try again later.',code:'RATE_LIMITED'},'application/json',req);return false}return true}
const EVIDENCE_KEY=(()=>{const raw=process.env.EVIDENCE_ENCRYPTION_KEY;if(!raw)return null;try{const b=Buffer.from(raw,'base64');return b.length===32?b:null}catch(e){return null}})();
function encryptEvidence(buf){if(!EVIDENCE_KEY)throw new Error('Evidence encryption key is not configured');const iv=crypto.randomBytes(12),c=crypto.createCipheriv('aes-256-gcm',EVIDENCE_KEY,iv);const enc=Buffer.concat([c.update(buf),c.final()]);return Buffer.concat([iv,c.getAuthTag(),enc])}
...
const server=http.createServer(async(req,res)=>{if(req.method==='OPTIONS'){const origin=req.headers.origin;if(origin&&ALLOWED_ORIGINS.has(origin)){res.writeHead(204,{...corsHeaders(req),'Access-Control-Allow-Methods':'GET,POST,OPTIONS','Access-Control-Allow-Headers':'Content-Type','Access-Control-Max-Age':'600'});return res.end()}return send(res,204,'', 'application/json',req)}try{
if(!rateGuard(req,res,'global',120))return;
if(req.url==='/api/auth/register'&&req.method==='POST'){if(!rateGuard(req,res,'auth-register',8,3600000))return;const x=await body(req,100*1024),email=normalizeEmail(x.email),password=String(x.password||'');if(!validEmail(email)||password.length<10||password.length>128)return send(res,400,{ok:false,error:'Use a valid email and a password of 10-128 characters.'},'application/json',req);const d=readUsers();if(d.users.some(u=>u.email===email))return send(res,409,{ok:false,error:'Account already exists. Please log in.'},'application/json',req);const hp=hashPassword(password);const user={id:'U-'+crypto.randomBytes(8).toString('hex'),email,passwordSalt:hp.salt,passwordHash:hp.hash,createdAt:new Date().toISOString()};d.users.push(user);writeUsers(d);setSession(res,{id:user.id,email:user.email});return send(res,201,{ok:true,user:{id:user.id,email:user.email}},'application/json',req)}
if(req.url==='/api/auth/login'&&req.method==='POST'){if(!rateGuard(req,res,'auth-login',10,900000))return;const x=await body(req,100*1024),email=normalizeEmail(x.email),password=String(x.password||'');const d=readUsers(),u=d.users.find(v=>v.email===email);if(!u||!verifyPassword(password,{salt:u.passwordSalt,hash:u.passwordHash}))return send(res,401,{ok:false,error:'Invalid email or password.'},'application/json',req);setSession(res,{id:u.id,email:u.email});return send(res,200,{ok:true,user:{id:u.id,email:u.email}},'application/json',req)}
if(req.url==='/api/auth/me'&&req.method==='GET'){const u=currentUser(req);return send(res,200,{ok:true,authenticated:!!u,user:u||null},'application/json',req)}
if(req.url==='/api/auth/logout'&&req.method==='POST'){clearSession(req,res);return send(res,200,{ok:true},'application/json',req)}
if(req.url==='/api/health'&&req.method==='GET')return send(res,200,{ok:true,service:'VAJRA NYAY BACKEND',groqConfigured:!!process.env.GROQ_API_KEY,groqModel:process.env.GROQ_MODEL||'openai/gpt-oss-20b',policeLiveConfigured:!!process.env.POLICE_LIVE_ENDPOINT,govApiConfigured:!!process.env.GOV_API_ENDPOINT,security:{rateLimiting:true,authentication:true,evidenceEncryptionConfigured:!!EVIDENCE_KEY},lawUpdateLastChecked:readLawDb().lastChecked,lawUpdateSource:'India Code + Legislative Department',time:new Date().toISOString()});
if(req.url==='/api/config'&&req.method==='GET')return send(res,200,{ok:true,groqConfigured:!!process.env.GROQ_API_KEY,groqModel:process.env.GROQ_MODEL||'openai/gpt-oss-20b',policeLiveConfigured:!!process.env.POLICE_LIVE_ENDPOINT,govApiConfigured:!!process.env.GOV_API_ENDPOINT,security:{rateLimiting:true,authentication:true,evidenceEncryptionConfigured:!!EVIDENCE_KEY}});
if(req.url==='/api/sos/chunk'&&req.method==='POST'){const user=requireAuth(req,res);if(!user)return;if(!EVIDENCE_KEY)return send(res,503,{ok:false,error:'Secure evidence storage is not configured on this server.'},'application/json',req);if(!rateGuard(req,res,'sos',30))return;const x=await body(req,20*1024*1024);if(!x.sessionId||!x.videoBase64)return send(res,400,{ok:false,error:'Missing SOS chunk'},'application/json',req);const safe=String(x.sessionId).replace(/[^a-zA-Z0-9_-]/g,'').slice(0,80);const owner=String(user.id).replace(/[^a-zA-Z0-9_-]/g,'');const dir=path.join(SOS_DIR,owner+'_'+safe);if(!fs.existsSync(dir))fs.mkdirSync(dir,{recursive:true,mode:0o700});const raw=Buffer.from(x.videoBase64,'base64');if(raw.length>12*1024*1024)return send(res,413,{ok:false,error:'Evidence chunk too large'},'application/json',req);const hash=crypto.createHash('sha256').update(raw).digest('hex');const encrypted=encryptEvidence(raw);const fn=String(Number.isFinite(Number(x.chunkIndex))?x.chunkIndex:0).padStart(8,'0')+'.bin';fs.writeFileSync(path.join(dir,fn),encrypted,{mode:0o600});fs.writeFileSync(path.join(dir,'manifest.jsonl'),JSON.stringify({ownerId:user.id,sessionId:safe,chunkIndex:x.chunkIndex,sha256:hash,createdAt:x.createdAt,mimeType:x.mimeType||'video/webm',encrypted:true})+'\n',{flag:'a',mode:0o600});const forwarded=await forwardPolice({...x,sha256:hash});return send(res,201,{ok:true,stored:true,sha256:hash,forwarded,encrypted:true},'application/json',req);}
if(req.url==='/api/evidence/stamp'&&req.method==='POST'){const user=requireAuth(req,res);if(!user)return;if(!rateGuard(req,res,'evidence-stamp',30))return;const x=await body(req,100*1024);if(!x.sha256)return send(res,400,{ok:false,error:'sha256 required'});return send(res,201,{ok:true,stampId:'ST-'+crypto.randomBytes(6).toString('hex').toUpperCase(),sha256:x.sha256,createdAt:new Date().toISOString(),anchoring:'HASH_TIMESTAMP_ONLY'});}
...
if(req.url==='/api/law-updates/check'&&req.method==='POST'){if(!rateGuard(req,res,'law-update-check',4,3600000))return;const db=await syncOfficialLawCatalog(true);return send(res,200,{ok:true,lastChecked:db.lastChecked,acts:(db.acts||[]).slice(0,100),errors:db.errors||[]});}
...
if(req.url==='/api/pocket-rights'&&req.method==='POST'){if(!rateGuard(req,res,'pocket-rights',20))return;
...
if(req.url.startsWith('/api/complaints')&&req.method==='GET'){if(!requireAuth(req,res))return;const d=read(),q=new URL(req.url,'http://localhost').searchParams.get('q')?.toLowerCase()||'';return send(res,200,(d.complaints||[]).filter(x=>!q||String(x.id).toLowerCase().includes(q)||String(x.category||'').toLowerCase().includes(q)))}
if(req.url==='/api/complaints'&&req.method==='POST'){const user=requireAuth(req,res);if(!user)return;if(!rateGuard(req,res,'complaints-write',20))return;const x=await body(req),d=read();d.complaints=d.complaints||[];const i=d.complaints.findIndex(v=>v.id===x.id);if(i>=0)d.complaints[i]={...d.complaints[i],...x};else d.complaints.push(x);write(d);return send(res,201,{ok:true,complaint:x})}
if(req.url.startsWith('/api/complaints/')&&req.method==='GET'){if(!requireAuth(req,res))return;const id=decodeURIComponent(req.url.split('/').pop()),x=(read().complaints||[]).find(v=>v.id===id);return x?send(res,200,x):send(res,404,{ok:false,error:'Case not found'})}
...
send(res,404,{ok:false,error:'Not found'})}catch(e){send(res,500,{ok:false,error:'Server error'},'application/json',req)}});
scheduleLawSync();
server.listen(PORT,()=>{console.log(`VAJRA NYAY: http://localhost:${PORT}`);if(!EVIDENCE_KEY)console.log('WARNING: EVIDENCE_ENCRYPTION_KEY is not configured; encrypted evidence storage is disabled.');});